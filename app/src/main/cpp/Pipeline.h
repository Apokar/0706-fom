...
class Fom {
public:
    explicit Fom(const std::string &fommodelDir,
                      const int cpuThreadNum, const std::string &cpuPowerMode);

    void Predict(const cv::Mat &source, std::string videoPath, std::vector<cv::Mat> &results,
                 double *predictKpTime, double *predictGenTime);
    ~Fom();

private:
    void KpPreprocess(const cv::Mat &rgbaImage);
    void GenPreprocess();
    void Postprocess(std::vector<RESULT> *results);
    void DrivingProcess();
...
private:
    int inputWidth_;
    int inputHeight_;
    std::shared_ptr<paddle::lite_api::PaddlePredictor> kp_predictor_;
    std::shared_ptr<paddle::lite_api::PaddlePredictor> generator_predictor_;
    std::vector<const float*> driving_jacobians;
    std::vector<const float*> driving_values;
    std::string videoPath_;
    int driving_num;

    std::shared_ptr<paddle::lite_api::PaddlePredictor> test_mul_fp16_predictor_;
    std::shared_ptr<paddle::lite_api::PaddlePredictor> test_mul_fp32_predictor_;
};


class Pipeline {
public:
  Pipeline(const std::string &modelDir, const std::string &fommodelDir, const std::string &labelPath,
           const int cpuThreadNum, const std::string &cpuPowerMode,
  ...
  int FomProcess(std::string imagePath, std::string videoPath);
