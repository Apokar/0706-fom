...
Java_com_baidu_paddle_lite_demo_fom_Native_nativeInit(
        JNIEnv *env, jclass thiz, jstring jModelDir, jstring jFomModelDir, jstring jLabelPath,
  ...
  std::string fommodelDir = jstring_to_cpp_string(env, jFomModelDir);
  ...
  return reinterpret_cast<jlong>(
          new Pipeline(modelDir, fommodelDir, labelPath, cpuThreadNum, cpuPowerMode, inputWidth,
...
Java_com_baidu_paddle_lite_demo_fom_Native_nativeRelease(
 ...
 */
JNIEXPORT jboolean JNICALL
Java_com_baidu_paddle_lite_demo_fom_Native_nativeProcess(
...
JNIEXPORT jboolean JNICALL
Java_com_baidu_paddle_lite_demo_fom_Native_nativeFomProcess(
        JNIEnv *env, jclass thiz, jlong ctx, jstring jimagePath, jstring jvideoPath) {
  if (ctx == 0) {
    return JNI_FALSE;
  }
  std::string imagePath = jstring_to_cpp_string(env, jimagePath);
  std::string videoPath = jstring_to_cpp_string(env, jvideoPath);
  Pipeline *pipeline = reinterpret_cast<Pipeline *>(ctx);
  return pipeline->FomProcess(imagePath, videoPath);
}
}
