package com.baidu.paddle.lite.demo.fom.preprocess;
...
public class Preprocess {
    ...
    public boolean process(Bitmap inputImage){
        if (channels == 3) {
            int[] channelIdx = null;
            ...
            for (int y = 0; y < height; y++) {
                for (int x = 0; x < width; x++) {
                    ...
                    float[] rgb = new float[]{(float) red(color), (float) green(color),
                            (float) blue(color)};
                    inputData[y * width + x] = (rgb[channelIdx[0]] - config.inputMean[0]) / config.inputStd[0];
                    inputData[y * width + x + channelStride[0]] = (rgb[channelIdx[1]] - config.inputMean[1]) / config.inputStd[1];
                    inputData[y * width + x + channelStride[1]] = (rgb[channelIdx[2]] - config.inputMean[2]) / config.inputStd[2];
                }
            }
        } else if (channels == 1) {
            for (int y = 0; y < height; y++) {
                for (int x = 0; x < width; x++) {
                    ...
                    inputData[y * width + x] = (gray - config.inputMean[0]) / config.inputStd[0];
                }
            }
        } else {
